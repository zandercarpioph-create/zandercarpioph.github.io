...
\newcommand{\proj}[2]{%
  \textbf{#1}\ \ {\small\textcolor{muted}{#2}}\\[1pt]
}
...
\vspace{9pt}
{\small
Mathematics undergraduate working on quantitative modelling: epidemic dynamics,
geospatial disease surveillance, and computational number theory. Seeking
actuarial and data science roles.
}
...
{\small Content production and campaign management for small businesses, run alongside full-time study.}

% ---------------------------------------------------------------- projects
\section{Quantitative Projects}

{\small
\proj{Dengue surveillance analysis, Baguio City}{R, QGIS}
Reconstructed barangay-level surveillance across 127 barangays and 23 reporting
weeks, joined on PSGC codes and mapped in QGIS, resolving an outbreak that peaked
at 1,778 reported cases and fell below 70 within six weeks. Found two defects that
silently corrupt naive analysis of the source files: counts mislabelled as rates,
and delimiter collisions inside area names.

\vspace{4pt}
\proj{Compartmental epidemic models}{R, deSolve, Shiny, JavaScript}
SIR and SEIR systems integrated with \texttt{deSolve} and published as interactive
Shiny applications, then reimplemented independently in JavaScript with a
fourth-order Runge-Kutta integrator. The two agree on peak infected, peak timing
and attack rate.

\vspace{4pt}
\proj{Cholera transmission with an environmental reservoir}{R, MATLAB}
...
\vspace{4pt}
\proj{Kaprekar triples, undergraduate thesis}{PARI/GP, LaTeX}
...
Languages & R, Python, MATLAB, C, JavaScript, PARI/GP \\[2pt]
Libraries & deSolve, Shiny, ggplot2, dplyr, tidyr, NumPy \\[2pt]
Tools     & QGIS, LaTeX, Git, Excel \\
\end{tabular}
}
